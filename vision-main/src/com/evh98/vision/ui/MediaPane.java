...
public class MediaPane {
	...
	public void render(SpriteBatch sprite_batch, ShapeRenderer shape_renderer, Sprite poster) {
		// Draw poster
		drawPoster(sprite_batch, poster);
		// Draw title
		drawTitle(sprite_batch, shape_renderer, Palette.DARK_GRAY);
	}
	
	public void renderAlt(SpriteBatch sprite_batch, ShapeRenderer shape_renderer, Sprite poster) {
		// Draw poster
		drawPoster(sprite_batch, poster);
		// Draw title
		drawTitle(sprite_batch, shape_renderer, screen_color);
	}
	
	private void drawPoster(SpriteBatch sprite_batch, Sprite poster) {
		sprite_batch.begin();
		Graphics.drawSprite(sprite_batch, poster, x, y, 730, 1082);
		sprite_batch.end();
	}
	
	private void drawTitle(SpriteBatch sprite_batch, ShapeRenderer shape_renderer, Color color) {
		shape_renderer.begin(ShapeType.Filled);
		shape_renderer.setColor(color);
		Graphics.drawRect(shape_renderer, x, y + 1080, 730, 260);
		shape_renderer.end();
		sprite_batch.begin();
		Graphics.drawText(sprite_batch, font, text, x + 365, y + 1210);
		sprite_batch.end();
	}
}